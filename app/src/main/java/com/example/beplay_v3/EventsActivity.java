...
public class EventsActivity extends AppCompatActivity {
    ...
    private Button backButton;   // bottom nav: Back
    private Button homeButton;   // bottom nav: Home
    ...
    // Map KEYCODE -> Button view (for dynamic phrases)
    ...
    // Tracks the last focused event button (our "center" in the loop)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tts = new TextToSpeech(this, status -> {
            if (status == TextToSpeech.SUCCESS) {
                ...
                tts.speak("Choose the event of your choice",
        ...
        if (homeButton != null) {
            ...
            homeButton.setOnKeyListener((v, keyCode, event) -> {
                ...
                if (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
                    // RIGHT: next in ring (Home -> Event -> Back -> Home ...)
                    ...
                    // LEFT: prev in ring (Home -> Back -> Event -> Home ...)
                ...
                // UP/DOWN: let system handle
                return false;
            });
        }

        // ===== Style + behavior: BACK button =====
        if (backButton != null) {
            styleBackButton(backButton);

            backButton.setOnClickListener(v -> {
                CategoriaActivity.REPLAY_INTRO_ON_RESUME = true;
        ...
        // Voice init + static phrase ("back", "home")
        buildKeycodePool();
        initVuzixSpeechClient();
        ...
        client.newCall(req).enqueue(new Callback() {
            ...
            @Override public void onResponse(Call call, Response response) throws IOException {
                ...
                runOnUiThread(() -> {
                    ...
                    for (int index = 0; index < finalEvents.length; index++) {
                        EventItem ev = finalEvents[index];
                        ...
                        // LEFT/RIGHT ring for ANY event: uses ring helpers
                        ...
                        // ===== VOICE: dynamic phrase for each event =====
    ...
    private View getRingNext(View current) {
        ...
        if (idx == -1) {
            // if somehow current not in ring, default to first
            return ring.get(0);
        }
    ...
    // Event row buttons – narrow glass style
    ...
    // Back / Home button style – same visual, wrap_content width
    ...
    private void speakCurrentlyFocusedItem() {
        containerButtons.postDelayed(() -> {
            View focused = containerButtons.findFocus(); // only within events container
            if (focused == null) {
                if (containerButtons.getChildCount() > 0) {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        // Static: "home" (Vuzix may map to KEYCODE_MOVE_HOME)
    ...
    protected void onResume() {
        ...
        // default: announce highlighted event
        speakCurrentlyFocusedItem();
    }
